  ...
  fs.readFile('/var/www/html/app/public_html/cloudrun/update/mapList.json', (err, data) => {
    if (err) {
      ...
      const leaderboard = fs.readFileSync(
          '/var/www/html/app/public_html/cloudrun/leaderboardInfo/leaderboard.json'
        ),
        leaderboardJson = JSON.parse(leaderboard.toString('utf8'));
      resInfo.leaderboard = leaderboardJson.rankType1
        .reverse()
        .filter((_res) => +_res.month <= +moment().format('YYYYMM'));
