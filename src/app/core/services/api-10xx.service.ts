...
export class Api10xxService {
   ...
   * Api-v2 1004-忘記密碼
   */
  fetchForgetpwd(body, newHeader: any): Observable<any> {
    const httpOptions = this.setNewHeader(newHeader);
   ...
   * Api-v2 1005-編輯帳密
   */
  fetchEditAccountInfo(body, newHeader: any): Observable<any> {
    const httpOptions = this.setNewHeader(newHeader);
  ...
  /**
   * api-v2 1011 編輯會員資料 ----> 建議使用userService.updateUserProfile()，可以順便更新已儲存的userProfile
   * @param body {any}-api 所需參數
   */
