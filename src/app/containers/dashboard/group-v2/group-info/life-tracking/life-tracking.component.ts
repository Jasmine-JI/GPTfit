...
export class LifeTrackingComponent implements OnInit, OnDestroy {
    ...
    this.reportService
      ...
      .subscribe((resArr) => {
        ...
        if (this.uiFlag.progress >= 10 && this.uiFlag.progress < 100) {
          ...
          const {
            group: { selectGroup: preSelectGroup },
  ...
  createRangeTrend(userId: number, userData: any) {
    ...
    userData.forEach((_tracking) => {
      ...
      regressionObj = this.handleRegression(
        regressionObj,
        'bodyWeight',
        bodyWeight,
        userId,
                ...
                if (!excludeKey.includes(key) && !isEffectCountKey) {
                  ...
                  switch (key) {
                    ...
                    case 'totalSleepSecond':
                      ...
                      addValue =
                        personData[_id][key] / personData[_id][`${addKey}EffectCount`] || 0;
                      break;
                    default:
                      addKey = key;
  ...
  setDefaultPersonCol(len: number) {
    this.personTableOpt = ['name', 'totalStep', 'BMI', 'fatRate', 'muscleRate', 'bodyAssessment'];
  ...
  subscribeScrollAndClick() {
    ...
    this.scrollAndClickEvent = mergeEvent.pipe(takeUntil(this.ngUnsubscribe)).subscribe((event) => {
      this.closeAllMenu();
    });
  }
  ...
  sortData(type: SettingObj, data: Array<any>, sortCategory: string, asc: boolean) {
    ...
    const showRatioKey = ['totalStep', 'totalFitTime', 'distance'];
