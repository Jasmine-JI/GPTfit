...
@Component({
  ...
  styleUrls: ['./max-speed.component.css']
})
export class MaxSpeedComponent implements AfterViewInit, OnChanges {
  @ViewChild('maxSpeedChartTarget')
  maxSpeedChartTarget: ElementRef;
    ...
    sportTypes.sort().map(_type => {
      ...
      this.datas
        ...
        .forEach(_data => {
          ...
          data[idx] = [_data.startTime.slice(0, 10), +_data.activities[0].avgMaxSpeed];
        });
      let name = '';
      let color = '';
      if (_type === '1') {
  ...
  initHchart() {
    ...
    this.chart1 = chart(this.maxSpeedChartTarget.nativeElement, options);
  }
}
