...
@Component({
  selector: 'app-average-speed',
  templateUrl: './average-speed.component.html',
  styleUrls: ['./average-speed.component.css']
})
export class AverageSpeedComponent implements OnChanges, AfterViewInit {
  @ViewChild('averageSpeedChartTarget')
  averageSpeedChartTarget: ElementRef;
    ...
    sportTypes.sort().map(_type => {
      ...
      this.datas
        ...
        .forEach(_data => {
          ...
          data[idx] = [_data.startTime.slice(0, 10), +_data.activities[0].avgSpeed];
        });
      let name = '';
      let color = '';
      if (_type === '1') {
  ...
  initHchart() {
    ...
    this.chart1 = chart(this.averageSpeedChartTarget.nativeElement, options);
  }
}
