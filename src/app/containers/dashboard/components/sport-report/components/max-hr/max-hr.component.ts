...
@Component({
  selector: 'app-max-hr',
  templateUrl: './max-hr.component.html',
  styleUrls: ['./max-hr.component.css']
})
export class MaxHRComponent implements AfterViewInit, OnChanges {
  @ViewChild('maxHRChartTarget')
  maxHRChartTarget: ElementRef;
    ...
    sportTypes.sort().map(_type => {
      ...
      this.datas
        ...
        .forEach(_data => {
          ...
          data[idx] = [_data.startTime.slice(0, 10), +_data.activities[0].avgMaxHeartRateBpm];
        });
      let name = '';
      let color = '';
      if (_type === '1') {
  ...
  initHchart() {
    ...
    this.chart1 = chart(this.maxHRChartTarget.nativeElement, options);
  }
}
