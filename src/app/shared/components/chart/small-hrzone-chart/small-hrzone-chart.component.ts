  ...
  drowChart() {
    ...
    setTimeout(() => {
      ...
      if (canvas.getContext) {
        ...
        const hrZonePercentage = this.data.map(_second => Math.round((_second / totalSecond) * 100));

        // 依最高佔比作為圖表最高點
        const maxSecond = this.data.reduce((prev, current) => prev < current ? current : prev);
        const hrZoneChartPercentage = this.data.map(_second => Math.round((_second / maxSecond) * 100));
