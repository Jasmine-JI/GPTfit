...
export class CloudrunMapComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(e: any): void {
    ...
    if (this.userList && e.userList) {
      ...
      if (this.isPreviewMode) {
        ...
        this.mapOpt.mapSource === 'google' ? this.handleGoogleMap(this.mapGpx) : this.handleBaiduMap(this.mapGpx);
  ...
  handleGoogleMap(gpx: Array<any>) {
    setTimeout(() => {
      ...
      // 監聽縮放大小
      this.googleObj.zoomListener = google.maps.event.addListener(this.googleObj.map, 'zoom_changed', () => {
...
class CustomBMapIcon extends BMap.Overlay {
   ...
   */
  draw() {
    const divPosition = this.map.pointToOverlayPixel(
      this.position
    );
